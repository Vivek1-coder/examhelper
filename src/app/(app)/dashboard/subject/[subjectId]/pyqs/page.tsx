...
import AddPyqs from "@/components/Add/AddPyqs";
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      const response = await axios.get<ApiResponse>(
        `/api/pyqs/get-pyq?subjectId=${subjectId}`
      );
    ...
    <div className="w-screen h-screen relative">
      ...
      <div className="w-full h-full flex justify-center items-center">
        <div className="w-3/4 h-3/4 flex flex-wrap justify-center gap-4">
          ...
          {!session?.user?.accessToken && !isPublic && !isPyqLoading && (
            <Gsignin />
          )}
          {pyqs.map((pyq) => (
            <PyqCard
              key={pyq._id as string}
